...
impl<'a> Search<'a> {
    ...
    pub fn set_time(&mut self, time_ms: u32, inc_ms: u32) {
        self.min_search_time = (time_ms as u64) * 30605 + (inc_ms as u64) * 8062;
        self.max_search_time = (time_ms as u64) * 88845 + (inc_ms as u64) * 560161;
    }
    ...
    pub fn alpha_beta(&mut self, mut alpha: i32, beta: i32, depth: i32, ply: usize) -> Option<i32> {
        ...
        if depth > 0 && !pv_node && !is_check && static_eval >= beta {
            ...
            if depth <= 7 {
                const STATIC_NULL_MOVE_MARGIN: i32 = 67;
            ...
            if depth >= 3 {
                ...
                let r = (595 + depth * 60 + 2 * (static_eval - beta) - 2 * improving as i32) >> 8;
            ...
            let eval = if best_move.is_none() || depth <= 0 {
                ...
                let lmr_depth =
                    if depth >= 2 && i >= 3 && !mov.flags().is_noisy() && !is_check && !gives_check
                    {
                        // Round towards -inf is fine
                        let reduction =
                            (61 + depth * 11 + i as i32 * 41 - improving as i32 * 112) / 256;
