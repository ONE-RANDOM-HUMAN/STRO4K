...
pub static mut STATIC_NULL_MOVE_MARGIN: i32 = 84;
pub static mut F_PRUNE_MARGIN: i32 = 114;
pub static mut PIECE_VALUES: [i32; 5] = [114, 425, 425, 648, 1246];
pub static mut DELTA_BASE: i32 = 97;
pub static mut DELTA_IMPROVING_BONUS: i32 = 39;
pub static mut LMR_DEPTH: f64 = 0.18955529530449658;
pub static mut LMR_MOVE: f64 = 0.13077897217303366;
pub static mut LMR_IMPROVING: f64 = -0.7704614426171933;
pub static mut NMP_BASE: f64 = 3.7503942439305984;
pub static mut NMP_DEPTH: f64 = 0.1723964148950344;
pub static mut NMP_IMPROVING: f64 = -0.5950016905881689;
    ...
    pub fn alpha_beta(&mut self, mut alpha: i32, beta: i32, depth: i32, ply: usize) -> Option<i32> {
        ...
        if depth > 0 && !pv_node && !is_check && static_eval >= beta {
            ...
            if depth >= 3 {
                ...
                let eval =
                    self.alpha_beta(-beta, -beta + 1, depth - r as i32 - 1, ply + 1);
        ...
        for i in 0..moves.len() {
            ...
            if f_prune && depth <= 0 {
                unsafe {
                    ...
                    if static_eval + capture + promo + DELTA_BASE + (improving as i32 * DELTA_IMPROVING_BONUS)
                        <= alpha
                        {
                            continue;
                        }
            ...
            let eval = if best_move.is_none() || depth <= 0 {
                ...
                let lmr_depth = if depth >= 3
                    ...
                    && !gives_check
                {
                    let lmr_reduction = unsafe {
                        f64::from(depth) * LMR_DEPTH
                            + i as f64 * LMR_MOVE
                            + f64::from(improving) * LMR_IMPROVING
                    };

                    let lmr_depth = depth - lmr_reduction as i32 - 1;
