...
pub const PIECE_VALUES: [i32; 6] = [146, 439, 439, 706, 1408, MAX_EVAL];
...
#[rustfmt::skip]
const MOBILITY_ATTACK_EVAL: [[Eval; 5]; 4] = [
    [
        Eval(  -6,   27),
        Eval(  -2,   -7),
        Eval(  50,   32),
...
fn side_mobility(
    ...
    for i in 0..4 {
        ...
        while side_pieces != 0 {
            ...
            for (enemy_pieces, attack) in enemy_pieces[0..5].iter().zip(MOBILITY_ATTACK_EVAL[i]) {
...
fn white_king_safety(king: Bitboard, pawns: Bitboard, phase: i16) -> Eval {
    ...
    if king & KS_AREA != 0 {
        let file = 0x80_8000;
        let index = ((pawns & file != 0) as usize) << 2
            | ((pawns & (file >> 1) != 0) as usize) << 1
            | ((pawns & (file >> 2) != 0) as usize);

        eval.accum(
            PAWN_SHIELD_EVAL[index].map(|x| (x * phase) >> 3),
            1,
        );
        ...
        eval.accum(
            PAWN_SHIELD_EVAL[index as usize].map(|x| (x * phase) >> 3),
            1,
        );
    }
